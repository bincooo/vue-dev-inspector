...
| 包 | 角色 | 构建方式 | 入口 |
|---|---|---|---|
| `@vue-dev-inspector/shared` | 内部依赖变量 | 抽一个 shared 包，打断 core↔client 的耦合/循环，输出 `dist/index.js` | `index.ts` |
...
```
demo  →  core  →  { shared, client, overlay }
                ↑
                └── core 自身 → { shared (workspace:*), overlay (workspace:*), client (workspace:*), @vue/compiler-sfc }
公共依赖
  core   ──┐
  client ──│
           ↓
