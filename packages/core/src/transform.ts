...
export function createInspectorTransform(
  ...
  return (node) => {
    ...
    if (el.tagType !== ElementTypes.ELEMENT && el.tagType !== ElementTypes.COMPONENT) return
    ...
    if (el.tagType === ElementTypes.COMPONENT && wrapSet && wrapSet.has(el.tag)) {
      const end = start + el.loc.source.length
      s.appendLeft(start, `<span ${attrName}="${filePath}:${line}:${column}" data-inspector-tag="${el.tag}" data-inspector-wrap style="display:contents">`)
    ...
    const tagAttr = el.tagType === ElementTypes.COMPONENT ? ` data-inspector-tag="${el.tag}"` : ''
    s.appendLeft(insertPos, ` ${attrName}="${filePath}:${line}:${column}"${tagAttr}`)
  }
}