...
function buildCfgJson(options: Required<DevInspectorOptions>, projectRoot: string): string {
  const cfg = {
  ...
  return {
    ...
    transform(code, id) {
      ...
      compileTemplate({
        ...
        id,
        compilerOptions: {
          nodeTransforms: [createInspectorTransform(s, template, relativePath, options.attrName, options.wrapComponents)],
        },
      })

      return s.hasChanged()
        ? { code: s.toString(), map: s.generateMap({ source: id, includeContent: true, hires: true }) }
        : null
    },
