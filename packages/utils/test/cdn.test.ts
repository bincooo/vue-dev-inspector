...
describe("buildCdnUrl", () => {
  ...
  it("throws when no builder is registered", () => {
    expect(() => buildCdnUrl("foo", "1.0.0", "./dist/x.js")).toThrow(
      "[vdi] cdn builder 未注册",
    );
  });
  ...
  it("joins URL = baseUrl + relPath when relPath has no leading ./", () => {
    ...
    expect(buildCdnUrl("lib", "1.0.0", "index.js")).toBe(
      "https://cdn.example.com/lib/index.js",
    );
  });
  ...
  it("uses baseUrl directly when relPath is empty", () => {
    ...
    expect(buildCdnUrl("lib", "1.0.0", "")).toBe(
      "https://cdn.example.com/full/path.js",
    );
  });
  ...
  it("preserves builder-side URL contract (custom CDN)", () => {
    ...
    expect(buildCdnUrl("@scope/lib", "2.0.0", "./dist/lib.js")).toBe(
      "https://unpkg.com/@scope/lib@2.0.0/dist/lib.js",
    );
  });
  ...
  it("throws when async builder returns a Promise (sync API contract)", () => {
    ...
    expect(() => buildCdnUrl("lib", "1.0.0", "./x.js")).toThrow(
      "[vdi] cdn builder 必须同步返回 string",
    );
  });
});
