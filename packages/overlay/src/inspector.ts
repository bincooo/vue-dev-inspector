...
export function createUI(): void {
  ...
  const deleteButton = createElement('div', '__vdi-action-btn __vdi-delete-btn', '×')
  deleteButton.onmouseenter = function () { deleteButton.style.background = '#dc2626' }
  deleteButton.onmouseleave = function () { deleteButton.style.background = '#ef4444' }
  const copyButton = createElement('div', '__vdi-action-btn __vdi-copy-btn', '⧉')
  copyButton.onmouseenter = function () { copyButton.style.background = '#2563eb' }
  copyButton.onmouseleave = function () { copyButton.style.background = '#3b82f6' }
  ...
  ;[hoverOverlay, selectOverlay, tagTip, contextMenu, deleteButton, copyButton, insertBeforeButton, insertAfterButton]
    .forEach(function (layer) { document.body.appendChild(layer) })
...
export function duplicateElement(element: HTMLElement): void {
  ...
  apiRequest('/duplicate-element', { method: 'POST', body: JSON.stringify({ file: pos.file, line: +pos.line, col: +pos.col }) })
    .then(function (response: any) {
      if (response && response.success) console.log('%c[Vue DevInspector]%c 元素已复制', 'color:#3b82f6;font-weight:bold', 'color:inherit')
    })
}
...
export function deleteElement(element: HTMLElement): void {
  ...
  apiRequest('/delete-element', { method: 'POST', body: JSON.stringify({ file: pos.file, line: +pos.line, col: +pos.col, tag }) })
    .then(function () { if (state.selectedElement === element) { state.selectedElement = null; redrawSelection() } element.remove() })
}

/** 定位 复制/删除 按钮到选中框右下角；定位 +/- 同级插入按钮到上下边框中点 */
export function positionActionButtons(): void {
  if (!state.selectedElement || !state.selectOverlay || state.selectOverlay.style.display === 'none') {
    hideAllActionButtons()
    return
  }
  const selectionRect = getLayoutBox(state.selectedElement)!.getBoundingClientRect()
  /* 复制/删除：右下角 */
  ...
  state.insertBeforeButton!.style.left = (selectionRect.left + selectionRect.width / 2 - 9) + 'px'
  state.insertBeforeButton!.style.top = (selectionRect.top - 9) + 'px'
  state.insertAfterButton!.style.display = 'flex'
  state.insertAfterButton!.style.left = (selectionRect.left + selectionRect.width / 2 - 9) + 'px'
...
export function hover(element: HTMLElement): void {
  ...
  if (element === state.selectedElement) state.hoverOverlay.style.display = 'none'
...
export function toggle(force?: boolean): void {
  ...
  if (!state.inspecting) {
    hide(); state.selectedElement = null; state.selectOverlay!.style.display = 'none'
    hideAllActionButtons()
  }
  /* 齿轮按钮：审查开启时隐藏，关闭时显示 */
  if (state.gearButton) state.gearButton.style.display = state.inspecting ? 'none' : 'flex'
}