...
describe("extensibility registry", () => {
  ...
  it("registerBtn adds to map and supports same-id overwrite", () => {
    ...
    const cb2 = vi.fn();
    registerBtn([
      { id: "a", icon: "X", label: "lbl", position: "right", onClick: cb1 },
    ]);
    registerBtn([
      { id: "a", icon: "Y", label: "lbl", position: "right", onClick: cb2 },
    ]);
  ...
  it("registerBtn returns unregister function that removes entries", () => {
    const off = registerBtn([
      { id: "x", icon: "1", label: "l", position: "right", onClick: () => {} },
    ]);
    expect(state.toolButtons.size).toBe(1);
    off();
  ...
  it("emitSelect with null attaches source: null", () => {
    ...
    emitSelect(null);
    expect(cb).toHaveBeenCalledWith(
      expect.objectContaining({ target: null, source: null }),
    );
  });
