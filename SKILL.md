...
| list-components | 扫描.vue | 否 |
| get-component-config | 读vite.config.ts的componentConfig物料目录 {componentEntries:[{name,icon,groups:[{group,items:[{tag,label,snippet,icon,imports}]}]}]} | 否 |
...
| update-block | 替/新建块 {kind:script/style,content,scoped} scoped仅新建style生效，如果get-block为空会新建块 | 是 |
